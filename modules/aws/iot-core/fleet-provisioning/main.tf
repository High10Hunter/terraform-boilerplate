...
data "aws_iam_policy_document" "iot_thing_policy" {
  ...
  statement {
    ...
    effect    = "Allow"
    resources = ["arn:aws:iot:${var.region}:${var.account_id}:client/*"]
  }
  statement {
    actions = [
      "iot:Publish",
    ...
    effect    = "Allow"
    resources = ["arn:aws:iot:${var.region}:${var.account_id}:topic/*"]
  }
  statement {
    actions = [
      "iot:Subscribe"
    ...
    effect    = "Allow"
    resources = ["arn:aws:iot:${var.region}:${var.account_id}:topicfilter/*"]
  }
}
...
data "aws_iam_policy_document" "claim_policy" {
  ...
  statement {
    ...
    effect = "Allow"
    resources = [
      "arn:aws:iot:${var.region}:${var.account_id}:topic/$aws/certificates/create-from-csr/*",
      "arn:aws:iot:${var.region}:${var.account_id}:topic/$aws/provisioning-templates/${aws_iot_provisioning_template.fleet.name}/provision/*",
    ]
  }
  statement {
    actions = [
    ...
    effect = "Allow"
    resources = [
      "arn:aws:iot:${var.region}:${var.account_id}:topicfilter/$aws/certificates/create-from-csr/*",
      "arn:aws:iot:${var.region}:${var.account_id}:topicfilter/$aws/provisioning-templates/${aws_iot_provisioning_template.fleet.name}/provision/*"
    ]
  }
}
...
resource "aws_iot_policy" "thing_policy" {
  name   = "${title(var.project)}${title(terraform.workspace)}FleetProvisioningThingPolicy"
...
resource "aws_iot_provisioning_template" "fleet" {
  name                  = "${title(var.project)}${title(terraform.workspace)}FleetProvisioningTemplate"
...
### Uncomment the below code to retrieve the certificate and keys to be used at local
# resource "local_file" "cert_claim" {
#   filename = "${local.base_name}_certificate_pem.cert"
#   content  = aws_iot_certificate.claim_cert.certificate_pem
# }

# resource "local_file" "cert_private_key" {
#   filename = "${local.base_name}_cert_private.key"
#   content  = aws_iot_certificate.claim_cert.private_key
# }

# resource "local_file" "cert_public_key" {
#   filename = "${local.base_name}_cert_public.key"
